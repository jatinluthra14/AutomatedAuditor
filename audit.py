...
def init_azblob(args: argparse.Namespace) -> None:
    blob = AZBlob(container_name=args.container_name, tenant_id=args.az_creds[0], client_id=args.az_creds[1], client_secret=args.az_creds[2], subscription_id=args.az_creds[3])
    blob.start()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--bucket-name', default='', metavar='BucketName', help='The Name of Bucket/Blob to Audit (Exclude to Audit All)')
    parser.add_argument('-c', '--container-name', default='', metavar='ContainerName', help='The Name of Container to Audit (Azure Only, Specify Blob using -b if needed)')
